	...
	def execute(m, query)
		...
		begin
			...
			def search(number)
				...
				"1,0You0,4Tube \"%s\" (%s); %s views; %s/5 (%s|%s) - http://youtu.be/%s | More results: %s" % [name, length, views, rating[0..2], likes, dislikes, id, @page_url.shorten]
			end
		
			m.reply search(1)
		rescue
			m.reply "1,0You0,4Tube Error"
		end
	end
end
